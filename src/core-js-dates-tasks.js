...
function dateToTimestamp(date) {
  ...
  return dateInput[Symbol.toPrimitive]('number');
}
...
function getTime(date) {
  const dateInput = new Date(date);
  const hours = dateInput.getHours().toString();
  const minutes = dateInput.getMinutes().toString();
  const seconds = dateInput.getSeconds().toString();
...
function getDayName(date) {
  ...
  return dayReference[dateInput.getDay()];
}
...
function getCountDaysInMonth(month, year) {
  ...
  const first = new Date(year, monthIndx, 1, 12);

  const possibleDays = [31, 30, 29, 28];
  for (let i = 0; i < possibleDays.length; i += 1) {
    const date = new Date(new Date(first).setDate(possibleDays[i]));
    if (date.getMonth() === monthIndx) return possibleDays[i];
  }
  return undefined;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const dateStartInput = new Date(dateStart);
  const dateEndInput = new Date(dateEnd);
  const dateStartMs = dateStartInput.valueOf();
  const dateEndMs = dateEndInput.valueOf();
  return (dateEndMs - dateStartMs + msPerDay) / msPerDay;
}
...
function isDateInPeriod(date, period) {
  ...
  if (
    startDate.valueOf() <= dateToCheck.valueOf() &&
    endDate.valueOf() >= dateToCheck.valueOf()
  ) {
    return true;
  }
  return false;
...
function formatDate(date) {
  ...
  const dateString = `${dateToFormat.getUTCMonth() + 1}/${dateToFormat.getUTCDate()}/${dateToFormat.getUTCFullYear()}`;
  ...
  const timeString = `${hours}:${minutes.toString().padStart(2, '0')}:${dateToFormat.getUTCSeconds().toString().padStart(2, '0')} ${amOrPm}`;
...
function getCountWeekendsInMonth(month, year) {
  ...
  const possibleWeekendDay = date;
  let weekendsCount = 0;
  while (date.getMonth() === month - 1) {
    if (
      possibleWeekendDay.getDay() === 0 ||
      possibleWeekendDay.getDay() === 6
    ) {
      weekendsCount += 1;
    }
    possibleWeekendDay.setDate(possibleWeekendDay.getDate() + 1);
  }
...
function getNextFridayThe13th(date) {
  ...
  if (dateStart.getDate() !== 13) {
    dateStart.setDate(13);
    if (dateStart.getDay() === 5) return dateStart;
  }
  while (dateStart.getDay() !== 5) {
...
function getQuarter(date) {
  ...
  const monthsInAQuarter = 12 / 4;
  for (let i = 1; i <= 4; i += 1) {
    if (month <= monthsInAQuarter * i) return i;
  }
  return undefined;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (date.valueOf() <= endDate.valueOf()) {
    for (let i = 1; i <= countWorkDays; i += 1) {
      ...
      if (date.valueOf() > endDate.valueOf()) break;
    }
